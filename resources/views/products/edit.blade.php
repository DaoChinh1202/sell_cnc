  ...
  <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="card mb-4"><div class="card-header bg-white py-3"><h2 class="h5 mb-0">Thông tin sản phẩm</h2></div><div class="card-body p-4">
      <div class="row g-3">
        ...
        <div class="col-md-6"><label for="productBrand" class="form-label">Thương hiệu</label><input type="text" class="form-control @error('brand') is-invalid @enderror" id="productBrand" name="brand" value="{{ old('brand', $product->brand) }}">@error('brand')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
    ...
    <div class="card mb-4"><div class="card-header bg-white py-3"><h2 class="h5 mb-0">Giá và tồn kho</h2></div><div class="card-body p-4"><div class="row g-3">
      <div class="col-md-4"><label for="productPrice" class="form-label">Giá</label><input type="number" class="form-control" id="productPrice" name="price" value="{{ old('price', $product->price) }}" min="0" step="0.01" required></div>
      <div class="col-md-4"><label for="productTax" class="form-label">Thuế (%)</label><input type="number" class="form-control" id="productTax" name="tax" value="{{ old('tax', $product->tax) }}" min="0" max="100" step="0.01"></div>
      <div class="col-md-4"><label for="productDiscount" class="form-label">Giảm giá (%)</label><input type="number" class="form-control" id="productDiscount" name="discount" value="{{ old('discount', $product->discount) }}" min="0" max="100" step="0.01"></div>
      <div class="col-md-4"><label for="productQuantity" class="form-label">Số lượng</label><input type="number" class="form-control" id="productQuantity" name="quantity" value="{{ old('quantity', $product->quantity) }}" min="0" required></div>
      <div class="col-md-4"><label for="productMinimumQuantity" class="form-label">Số lượng tối thiểu</label><input type="number" class="form-control" id="productMinimumQuantity" name="minimum_quantity" value="{{ old('minimum_quantity', $product->minimum_quantity) }}" min="0"></div>
      <div class="col-md-4"><label for="productUnit" class="form-label">Đơn vị</label><input type="text" class="form-control" id="productUnit" name="unit" value="{{ old('unit', $product->unit) }}" required></div>
    </div></div></div>
